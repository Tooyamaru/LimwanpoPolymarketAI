...
- [Polymarket business model](polymarket-business-model.md) — Authoritative rules: prediction market only; CLOSE_POSITION is valid decision type (not yet in code); BNB is target asset (not in settings); multiple entries per market allowed by exposure rules not position count
