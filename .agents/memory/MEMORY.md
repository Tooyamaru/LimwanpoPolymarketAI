...
- [Polymarket business model](polymarket-business-model.md) — LimwanpoAI: prediction market only; CLOSE_POSITION valid decision type (not yet in code); BNB target asset (settings shows XRP); multiple entries per market allowed by exposure rules not position count
