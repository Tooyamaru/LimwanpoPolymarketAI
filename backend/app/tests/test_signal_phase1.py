...
class TestMakeDecision:
    ...
    def test_medium_score_watch(self):
        # SCORE_WATCH=20, SCORE_OPEN=30 → scores in [20, 30) map to WATCH
        decision, _ = _make_decision(
            score=25.0, direction="BUY_YES", spread_yes=0.01
        )
    ...
    def test_confidence_gate_only_for_open_long(self):
        ...
        decision, _ = _make_decision(
            score=25.0,               # in WATCH band [20, 30) — below SCORE_OPEN
            direction="BUY_YES",
            spread_yes=0.01,
            signal_confidence=5.0,    # very low, but gate only applies at score >= SCORE_OPEN
        )
        assert decision == "WATCH"
