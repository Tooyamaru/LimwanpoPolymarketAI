...
async def test_portfolio_pnl_schema(client: AsyncClient) -> None:
    ...
    assert "average_unrealized_pnl" in data
    assert "closed_positions" in data
...
async def test_portfolio_pnl_types(client: AsyncClient) -> None:
    ...
    data = response.json()
    assert isinstance(data["open_positions"], int)
    assert isinstance(data["total_unrealized_pnl"], float)
    assert isinstance(data["average_unrealized_pnl"], float)
    assert isinstance(data["closed_positions"], int)
    assert isinstance(data["total_realized_pnl"], float)
