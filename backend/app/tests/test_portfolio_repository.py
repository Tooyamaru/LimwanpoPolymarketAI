...
@pytest.mark.anyio
async def test_pnl_summary_includes_partial_unrealized(db_session: AsyncSession) -> None:
    """PARTIAL lots still carry open contracts → must contribute to total_live_state.

    A partial exit reduces remaining_quantity and books some realized PnL, but the
    remainder is still in the market and accrues unrealized PnL.  Excluding PARTIAL
    from the unrealized aggregation understates total_live_state and breaks the
    Cumulative Outcome formula (realized + unrealized).
    """
    db_session.add_all([
        # Fully open lot
        _make_position(order_id=1, condition_id="c1", status="OPEN",  unrealized_pnl=0.10),
        # Partially closed lot: has both realized and remaining unrealized
        _make_position(order_id=2, condition_id="c2", status="PARTIAL",
                       unrealized_pnl=0.05, realized_pnl=0.03),
        # Fully closed lot: zero unrealized, only realized
        _make_position(order_id=3, condition_id="c3", status="CLOSED",
                       unrealized_pnl=None, realized_pnl=0.20),
    ])
    await db_session.commit()

    summary = await get_pnl_summary(db_session)

    # Unrealized must include both OPEN and PARTIAL lots
    assert summary["open_positions"] == 2, (
        "open_positions should count OPEN + PARTIAL lots (both carry open exposure)"
    )
    assert abs(summary["total_unrealized_pnl"] - 0.15) < 1e-6, (
        f"total_unrealized should be 0.10 + 0.05 = 0.15, got {summary['total_unrealized_pnl']}"
    )

    # Resolution Result includes CLOSED and PARTIAL realized PnL
    assert summary["closed_positions"] == 2, (
        "closed_positions counts CLOSED + PARTIAL (both have crystallised gains)"
    )
    assert abs(summary["total_realized_pnl"] - 0.23) < 1e-6, (
        f"total_realized should be 0.20 + 0.03 = 0.23, got {summary['total_realized_pnl']}"
    )

    # Cumulative Outcome = realized + unrealized — must be consistent
    cumulative = summary["total_realized_pnl"] + summary["total_unrealized_pnl"]
    assert abs(cumulative - 0.38) < 1e-6, (
        f"Cumulative Outcome should be 0.15 + 0.23 = 0.38, got {cumulative}"
    )


@pytest.mark.anyio
async def test_pnl_summary_all_lots_closed_unrealized_is_zero(db_session: AsyncSession) -> None:
    """When all lots are CLOSED, total_live_state must be 0 (card PnL returns 0)."""
    db_session.add_all([
        _make_position(order_id=1, condition_id="c1", status="CLOSED",
                       unrealized_pnl=None, realized_pnl=0.15),
        _make_position(order_id=2, condition_id="c2", status="CLOSED",
                       unrealized_pnl=None, realized_pnl=-0.05),
    ])
    await db_session.commit()
    ...
    assert summary["open_positions"] == 0
    assert summary["total_unrealized_pnl"] == 0.0
    assert abs(summary["total_realized_pnl"] - 0.10) < 1e-6
