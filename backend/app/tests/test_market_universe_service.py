...
@pytest.mark.anyio
async def test_series_catalog_has_4_entries():
...
@pytest.mark.anyio
async def test_series_catalog_is_5m_only():
    timeframes = {entry["timeframe"] for entry in SERIES_CATALOG}
    assert timeframes == {"5m"}, "5M-ONLY: SERIES_CATALOG must contain only 5m timeframe"
...
@pytest.mark.anyio
async def test_sync_processes_all_4_series():
    """5M-ONLY: sync must process exactly 4 series (BTC/ETH/SOL/XRP 5m)."""
    svc = MarketUniverseService()
    mock_client = MagicMock()
    ...
    assert result["series_processed"] == 4
    await svc.close()
