...
Decision rules (applied in order):
  ...
  4. score >= SCORE_OPEN (30, AMM-init calibrated)
...
# Entry gate thresholds — sourced from settings for transparency.
# All values are intentionally calibrated for the Polymarket AMM-init phase.
# See settings.py (STRATEGY_SCORE_OPEN, STRATEGY_MIN_SIGNAL_CONFIDENCE, etc.)
# for full rationale and the REVIEW_REQUIRED flag.
# This is NOT a risk bypass — Risk Engine (Layer 9) is the primary hard gate.
SCORE_OPEN: float = settings.STRATEGY_SCORE_OPEN
SCORE_WATCH: float = settings.STRATEGY_SCORE_WATCH
SPREAD_THRESHOLD: float = settings.STRATEGY_SPREAD_THRESHOLD

# Phase 1: Signal confidence gates (AMM-init calibrated — see settings.py)
MIN_SIGNAL_CONFIDENCE: float = settings.STRATEGY_MIN_SIGNAL_CONFIDENCE
