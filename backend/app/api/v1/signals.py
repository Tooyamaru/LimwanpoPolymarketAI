...
# ── TTL caches ────────────────────────────────────────────────────────────────
# The dashboard polls /latest and /active every 30 s; the cache prevents
# redundant DB round-trips when multiple tabs or polling bursts overlap.
_SIGNAL_CACHE_TTL: float = 30.0
_latest_cache: dict = {"ts": 0.0, "limit": 0, "data": None}
_active_cache: dict = {"ts": 0.0, "limit": 0, "data": None}
...
async def get_latest_signals(
    ...
    now = _time.monotonic()
    if (
        _latest_cache["data"] is not None
        and _latest_cache["limit"] == limit
        and now - _latest_cache["ts"] < _SIGNAL_CACHE_TTL
    ):
        return _latest_cache["data"]

    signals = await repo.get_latest_signals(session, limit=limit)
    result = [SignalResponse.model_validate(s) for s in signals]
    _latest_cache["ts"] = now
    _latest_cache["limit"] = limit
...
async def get_active_market_signals(
    ...
    now = _time.monotonic()
    if (
        _active_cache["data"] is not None
        and _active_cache["limit"] == limit
        and now - _active_cache["ts"] < _SIGNAL_CACHE_TTL
    ):
    ...
    _active_cache["limit"] = limit
    _active_cache["data"] = result
    return result
