...
async def get_latest_active_markets(
    ...
    Time guards (mirrors get_active_universe): only markets whose trading
    window is currently open (start_time <= now < end_time) are included.
    Markets with NULL timestamps pass through so legacy rows are not dropped.
    """
