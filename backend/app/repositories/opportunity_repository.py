...
async def get_all_opportunities(
    ...
    """Return opportunity rows ordered by score descending.

    Phase 9C fix: defaults to `active_only=True` so callers never see
    opportunities computed for markets whose universe status has since
    moved to "upcoming"/"expired" (the Opportunity Engine only ever writes
    one UPSERT row per condition_id and rows are not deleted when a market
    rolls off the active universe, so without this filter every downstream
    consumer — Strategy Engine, Portfolio Allocation, and this API — could
    act on stale data for a market that is no longer tradable).

    When `limit` is None (the default) all matching rows are returned —
    this preserves the original behaviour for internal callers such as
    StrategyEngine and the stats endpoint.  API-facing callers should pass
    an explicit limit so the response payload stays bounded.

    Pass `active_only=False` only for diagnostic/debug tooling that
    explicitly needs to inspect stale rows.
    """
    stmt = select(Opportunity).where(Opportunity.opportunity_score >= min_score)

    if active_only:
        active_ids_stmt = select(MarketUniverse.condition_id).where(
        ...
        if not active_ids:
            return []
        stmt = stmt.where(Opportunity.condition_id.in_(active_ids))

    stmt = stmt.order_by(desc(Opportunity.opportunity_score))
...
async def get_top_opportunities(
    ...
    Phase 9C fix: `active_only=True` by default — see `get_all_opportunities`
    for the rationale (this is the same stale-data hazard, just with a
    hard-coded LIMIT instead of an optional one).
    """
    stmt = select(Opportunity).where(Opportunity.opportunity_score >= min_score)
    ...
    stmt = stmt.order_by(desc(Opportunity.opportunity_score)).limit(limit)
    result = await session.execute(stmt)
    return list(result.scalars().all())
